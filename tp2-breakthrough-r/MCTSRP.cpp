...
MCTSNode* bt_t::mcts_selection(MCTSNode* node, bt_t& board) {
    ...
    for(MCTSNode* child : node->children) {
        float score = child->wins / child->visits + sqrt(2*log(node->visits)/child->visits);
        if(score > bestScore) {
            bestScore = score;
            bestNode = child;
        }
    }
    board.play(bestNode->board.moves[0]);
    return mcts_selection(bestNode, board);
}

void bt_t::mcts_expansion(MCTSNode* node, bt_t& board) {
    std::vector<bt_move_t> validMoves;
    board.update_moves();
    for(bt_move_t move : board.moves) {
        if(board.can_play(move)) {
            validMoves.push_back(move);
        }
    }
    for(bt_move_t move : validMoves) {
...
bt_move_t bt_t::mcts(double time_limit) {
    ...
    MCTSNode* node = mcts_selection(root, *this);
    while(std::chrono::steady_clock::now() - start < std::chrono::duration<double>(time_limit)) {
        mcts_expansion(node, *this);
        bool result = mcts_simulation(*this);
